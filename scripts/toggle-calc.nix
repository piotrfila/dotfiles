{pkgs, ...}: {
  home.packages = with pkgs; [
    jq
    (writeShellScriptBin "toggle-calc" ''
      pidq=$(hyprctl clients -j | jq '[.[] | select(.workspace.name | contains("special:q")) | select(.class | contains("qalculate"))][0] | "\(.pid).\(.monitor)"')
      pidq=''${pidq//\"}
      echo $pidq
      ...
      pidf=$(hyprctl activewindow -j | jq '"\(.pid).\(.monitor)"')
      pidf=''${pidf//\"}
      pidf_saved="/tmp/pf-pidf"

      # check if special workspace is visible
      special=$(hyprctl monitors -j | jq '.[] | "w\(.specialWorkspace.name).\(.id)"')
      special=''${special//\"}
